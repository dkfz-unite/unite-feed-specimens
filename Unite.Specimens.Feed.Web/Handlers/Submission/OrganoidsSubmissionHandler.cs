...
public class OrganoidsSubmissionHandler
{
    ...
    private readonly Models.Specimens.Converters.OrganoidModelConverter _modelConverter;

    private readonly ILogger _logger;

    public OrganoidsSubmissionHandler(
        SpecimensWriter dataWriter,
        SpecimenIndexingTasksService tasksService,
        SpecimensSubmissionService submissionService,
        TasksProcessingService tasksProcessingService,
        ILogger<OrganoidsSubmissionHandler> logger)
    {
        ...
        _modelConverter = new Models.Specimens.Converters.OrganoidModelConverter();
    }
    ...
    private void ProcessSubmissionTasks()
    {
        ...
        _tasksProcessingService.Process(SubmissionTaskType.ORG, TaskStatusType.Prepared, 1, (tasks) =>
        {
            ...
            _logger.LogInformation("Processed organoids data submission in {time}s", Math.Round(stopwatch.Elapsed.TotalSeconds, 2));
    ...
    {
        var submittedData = _submissionService.FindOrganoidsSubmission(submissionId);
        ...
        _submissionService.DeleteOrganoidsSubmission(submissionId);
